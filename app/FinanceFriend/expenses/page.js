...
function ExpensesContent() {
  ...
  const calculateTotalForGroup = (expenses) => {
    return expenses.reduce((sum, expense) => {
      const category = categories.find(c => c.id === expense.categoryId);
      if (category?.name === "Income") return sum;
                    ...
                    return (
                      <div key={expense.id} className="py-3 flex justify-between items-center">
                        <div className="flex items-center gap-3">
                          ...
                          >
                            <i className={`${category?.icon || "fi fi-rr-wallet"} text-${category?.color?.substring(1) || "94a3b8"}`}></i>
                          </div>
                          <div>
